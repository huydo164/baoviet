			...
			<div class="row">
				<div class="col-xs-12">
						...
						<form id="frmSearch" method="GET" action="" class="frmSearch" name="frmSearch">
							<div class="panel-body panel-search">
								<div class="form-group col-sm-2">
									...
									<div>
										<input type="text" class="form-control input-sm" name="type_title" @if(isset($search['type_title']) && $search['type_title'] !='')value="{{$search['type_title']}}"@endif>
									</div>
								</div>
							...
							<div class="panel-footer text-right">
								<a class="btn btn-default btn-sm" class="reset" href="{{route('admin.type')}}"><i class="fa fa-recycle"></i>Bỏ lọc</a>
								<a class="btn btn-danger btn-sm" href="{{FuncLib::getBaseUrl()}}admin/type/edit"><i class="ace-icon fa fa-plus-circle"></i>Thêm mới</a>
					...
					@if(sizeof($data) > 0)
						...
						@endif
						<br>
						<form id="formListItem" method="POST" action="{{FuncLib::getBaseUrl()}}admin/type/delete" class="formListItem" name="txtForm">
							<table class="table table-bordered table-hover">
								...
								@foreach($data as $k=>$item)
									<tr>
										...
										<td>
											<label class="pos-rel">
												<input class="ace checkItem" name="checkItem[]" value="{{$item['type_id']}}" type="checkbox">
												<span class="lbl"></span>
											</label>
										</td>
										<td>{{$item['type_title']}}</td>
										<td>{{$item['type_keyword']}}</td>
										<td>{{$item['type_order_no']}}</td>
										<td>{{date('d/m/Y', $item['type_created'])}}</td>
										<td>
										...
										<td>
											<a href="{{FuncLib::getBaseUrl()}}admin/type/edit/{{$item['type_id']}}" title="Cập nhật">
