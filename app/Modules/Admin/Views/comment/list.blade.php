                            ...
                            <form id="frmSearch" method="GET" action="" class="frmSearch" name="frmSearch">
                                <div class="panel-body panel-search">
                                    <div class="form-group col-lg-2">
                                        ...
                                        <div>
                                            <input type="text" class="form-control input-sm" name="comment_title" @if(isset($search['comment_name']) && $search['comment_name'] !='')value="{{$search['comment_name']}}"@endif>
                                        </div>
                                    </div>

                                </div>
                                <div class="panel-footer text-right">
                                    <a class="btn btn-default btn-sm" class="reset" href="{{route('admin.comment')}}"><i class="fa fa-recycle"></i>Bỏ lọc</a>
                                    <a class="btn btn-danger btn-sm" href="{{FuncLib::getBaseUrl()}}admin/comment/edit"><i class="ace-icon fa fa-plus-circle"></i>Thêm mới</a>
                                ...
                                <table class="table table-bordered table-hover">
                                    ...
                                    @foreach($data as $k=>$item)
                                        <tr>
                                            ...
                                            <td><a target="_blank" title="{{$item->rating_name}}" href="">{{$item['comment_name']}}</a></td>
                                            <td><a target="_blank" title="{{$item->rating_email}}" href="">{{$item['comment_email']}}</a></td>
                                            <td><a target="_blank" title="{{$item->rating_detail}}" href="">{{$item['comment_detail']}}</a></td>
                                            <td><a target="_blank" title="{{$item->rating_ip}}" href="">{{$item['comment_ip']}}</a></td>
                                            <td>
                                                <a target="_blank" title="{{$item->rating_status}}" href="">
                                                    {{($item['comment_status']==0) ? 'Ẩn' : 'Hiện' }}
                                                </a>
                                            </td>
                                            <td>
                                                <a href="{{FuncLib::getBaseUrl()}}admin/rating/edit/{{$item['comment _id']}}" title="Cập nhật">
